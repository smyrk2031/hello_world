...
// サブパス: .env / .env.production の VITE_APP_BASE_PATH を読む（vite.config 実行時は process.env に .env が入らないため loadEnv を使用）
export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, process.cwd(), '')
  const appBase = (env.VITE_APP_BASE_PATH || process.env.VITE_APP_BASE_PATH || '').trim().replace(/\/+$/, '')
